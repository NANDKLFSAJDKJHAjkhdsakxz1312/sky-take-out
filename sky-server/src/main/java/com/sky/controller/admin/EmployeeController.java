...
public class EmployeeController {
    ...
    public Result<PageResult> select(EmployeePageQueryDTO employeePageQueryDTO){
        log.info("查询用户：{}",employeePageQueryDTO);
    ...
    public Result startOrStop(@PathVariable Integer status, Long id){
        log.info("启用/禁用用户：{}，{}",status,id);
        employeeService.startOrStop(status,id);
        return Result.success();
    }
    @ApiOperation("查询回显")
    @GetMapping("/{id}")
    public Result<Employee> updateSelect(@PathVariable Long id ){
        log.info("返回显示用户：{}",id);
        Employee employee = employeeService.updateSelect(id);
